...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # frontend URL in prod
...
class SearchRequest(BaseModel):
    text: str
    location: str  # "all" or a location string
...
async def search(request: SearchRequest):
    ...
    if request.location == "all":
        ...
        for line in lines:
            my_dict[f"{line}"] = get_score_and_short_summary(request.text,line)
        return my_dict
        
    else:
        response = get_score_and_short_summary(request.text,request.location)
        return response
